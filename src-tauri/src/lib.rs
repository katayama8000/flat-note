...
fn db_url() -> String {
    // Prefer the value baked in at compile time (production builds).
    // Fall back to the runtime env var (dev) then the local SQLite default.
    if let Some(url) = option_env!("FLAT_NOTE_DB_URL") {
        if !url.is_empty() {
...
fn db_auth_token() -> String {
    if let Some(token) = option_env!("FLAT_NOTE_DB_AUTH_TOKEN") {
        if !token.is_empty() {
            return token.to_string();
        }
    }
    std::env::var("FLAT_NOTE_DB_AUTH_TOKEN").unwrap_or_default()
}
